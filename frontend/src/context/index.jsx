...
export function AuthProvider({children}) {
  ...
  const [token, setToken] = useState(() => localStorage.getItem('familyprint_token'))
  ...
  function saveSession(d) { localStorage.setItem('familyprint_token',d.access_token); setToken(d.access_token); setUser(d.user) }
  function logout() { localStorage.removeItem('familyprint_token'); setToken(null); setUser(null) }
...
export function CartProvider({children}) {
  ...
  useEffect(()=>{ const s=localStorage.getItem('familyprint_cart'); if(s) dispatch({type:'LOAD',items:JSON.parse(s)}) },[])
  useEffect(()=>{ localStorage.setItem('familyprint_cart',JSON.stringify(cart)) },[cart])
